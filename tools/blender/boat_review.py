#!/usr/bin/env python3
"""boat_review.py — review renders for UNDERTOW boat candidates.

  blender --background --python tools/blender/boat_review.py -- \
      --boat public/assets/candidates/boat-a.glb \
      --out tools/review-boat-a.png [--keeper public/assets/keeper.glb] [--bbox-only]

Renders the boat at a 3/4 LOW angle on a near-black stage. With --keeper the
prepped 1.8 m keeper is stood amidships on the boat's inner floor (found by
ray-casting up through the hull) as the scale/fit check.

Always prints '[bbox] <name> x=.. y=.. z=.. tris=..' for the boat.
"""
...
def parse_args(argv):
    ...
    ap.add_argument("--boat", required=True)
...
def inner_floor_z(obj, lo, hi):
    ...
    Cast up through the hull at its centre and take the lowest hit that sits at
    least 12% of the boat's height above the bbox floor. The threshold is what
    skips the hull's own exterior bottom and — on the image-mode candidate —
    the flat slab of sea that Tripo reconstructed around the waterline, either
    of which would otherwise stand the keeper below the deck.
    """
...
def build_stage():
    ...
    # warm lantern key (as if hung off the mast), cool moon rim, soft fill
...
def render(subjects, out, res, samples):
    scn = bpy.context.scene
    ...
    # 3/4 LOW angle: well round the bow quarter, camera barely above the sheer.
    look = mathutils.Vector((0.78, -0.60, 0.26)).normalized()
    fov = 2 * math.atan(18.0 / 55.0)
    dist = radius / math.tan(fov / 2) * 1.18
...
def main():
    ...
    d = hi - lo
    print(f"[bbox] {a.boat} x={d.x:.3f} y={d.y:.3f} z={d.z:.3f} tris={tri_count(boat)}")
    if a.bbox_only:
        return 0
    ...
    if a.keeper:
        ...
        # amidships: hull centre in the long axis, centred across the beam
        kp.location = (
        ...
        print(f"[keeper] floor z={fz:.3f} height={(khi - klo).z:.3f}")
        subjects.append(kp)

    build_stage()
    render(subjects, a.out, a.res, a.samples)
    return 0
